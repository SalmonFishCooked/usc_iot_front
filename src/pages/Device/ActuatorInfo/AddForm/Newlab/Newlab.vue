<template>
  <t-form ref="form" :rules="config.formRule" :data="formData" @submit="onSubmit">
    <t-form-item label="传感类型" name="SensorType">
      <t-select v-model="formData.SensorType" :options="config.options.optionsSensorTypeNewLab" placeholder="请选择传感类型" />
    </t-form-item>
...
const onSubmit = async ({ validateResult, firstError, e }) => {
  if (!btnLoading.value) {
    ...
    if (validateResult === true) {
      const data = await api.actuator.createActuator({...formData.value, ...{DeviceID: myDeviceID.data, Type: 1}})
      if (data) {
