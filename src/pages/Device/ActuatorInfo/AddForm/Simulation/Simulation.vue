<template>
  <t-form ref="form" :rules="config.formRule" :data="formData" @submit="onSubmit">
    <t-form-item label="传感类型" name="SensorType">
      <t-select v-model="formData.SensorType" :options="config.options.optionsSensorTypeSimulation" placeholder="请选择传感类型" />
    </t-form-item>
...
<!--        <t-option v-for="item in ['1','2','3','4','5','7']" :key="item" :value="item" :label="item"></t-option>-->
<!--      </t-select>-->
<!--    </t-form-item>-->
...
const onSubmit = async ({ validateResult, firstError, e }) => {
  if (!btnLoading.value) {
    ...
    if (validateResult === true) {
      const data = await api.actuator.createActuator({...formData.value, ...{DeviceID: myDeviceID.data, Type: 3}})
      if (data) {
