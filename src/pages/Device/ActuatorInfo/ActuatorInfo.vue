  ...
  <div class="bg-white shadow-box p-4 rounded-md">
    <t-loading :loading="loading">
      <div class="flex items-center space-x-2 mb-4">
        <img class="w-8" src="../../../assets/svg/actuator.svg" alt="">
        <p class="font-bold text-lg flex-1">执行器</p>
      ...
      <t-alert v-show="!tableData.data.length" theme="info" message="当前没有任何执行器可用。" />
...
async function handleRefresh(msg, data) {
  if (!loading.value) {
    ...
    const data = await api.actuator.getActuatorInfo({...{DeviceID: DeviceID.data, ApiTag: '', Type: tabs.value}, ...{Page: pagination.data.current, PageSize: pagination.data.pageSize}})
    if (data) {
    ...
    const confirmDia = DialogPlugin({
      ...
      onConfirm: async ({ e }) => {
        ...
        for (let i of selectVal.value) {
          const data = await api.actuator.deleteActuator({DeviceID: DeviceID.data, ApiTag: i})
          if (!data) {
            flag = 0
          }
        }
